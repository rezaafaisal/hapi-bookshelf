...
const addBook = (res, h) => {
   ...
   if(success){
      return h.response(replySuccess("success", "Buku berhasil ditambahkan", {bookId: id})).code(201)
   }
   return h.response(
...
const editBook = (res, h) => {
   ...
   let finished = false
   if(readPage > pageCount) return h.response(replyError("fail", "Gagal memperbarui buku. readPage tidak boleh lebih besar dari pageCount")).code(400)
   if(pageCount == readPage) finished = true
...
const deleteBook = (res, h) => {
   ...
   const index = books.findIndex(item => item.id == id)
   if(books.filter((book)=> book.id == id).length == 0){
      return h.response(replyError("fail", "Buku gagal dihapus. Id tidak ditemukan")).code(404)
   }
   if(index >= 0){
      books.splice(index, 1)
      return h.response(replySuccess("success", 'Buku berhasil dihapus')).code(200)
   }
      return h.response(replyError("error", 'Buku gagal dihapus')).code(500)
